...
sealed class MucEvents : Event(TYPE) {
    ...
    companion object : EventDefinition<MucEvents> {
        ...
        override val TYPE = "lib.takina.core.xmpp.modules.muc.MucEvents"
    }
}
...
sealed class MucRoomEvents(val room: Room) : Event(TYPE) {
    ...
    companion object : EventDefinition<MucRoomEvents> {
        ...
        override val TYPE = "lib.takina.core.xmpp.modules.muc.MucRoomEvents"
    }
}
...
//	private val log = LoggerFactory.logger("lib.takina.core.xmpp.modules.muc.MUCModule")
    ...
    fun decline(invitation: Invitation, reason: String? = null): RequestBuilder<Unit, Message> {
        if (invitation.direct) throw TakinaException("Direct invitation should be silently ignored.")
        return context.request.message {
