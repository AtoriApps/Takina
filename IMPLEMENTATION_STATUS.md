...
1. **OMEMO 端到端加密 (XEP-0384 + XEP-0420)**：`部分实现`
    * 已支持 `urn:xmpp:omemo:2` 设备列表与 bundle 的 publish/fetch、材料缓存、消息加密/解密收发链路（`OmemoComponent`），并接入统一消息 DSL（`message { encryption { method=OMEMO ... } }`）
    * *剩余工作*：与标准 Signal Session 的严格互操作（含完整会话棘轮）、多端状态同步策略与跨端持久化恢复
2. **P0 协议族的严格语义补齐**
