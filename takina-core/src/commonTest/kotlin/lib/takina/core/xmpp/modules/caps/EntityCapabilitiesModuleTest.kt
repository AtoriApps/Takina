...
class EntityCapabilitiesModuleTest {
	...
	fun test_more_complex_validate_verification_string() {
		val stanza = parseXML(
			"""
			<iq xmlns="jabber:client" from="sure.im" to="receiver@sure.im/1244996408-takina-32233" id="ZZBfgtF7K7fR1bCbv5QtgfzD"
			    type="result">
			    <query xmlns="http://jabber.org/protocol/disco#info"
			        ...
			        <feature var="takina:push:away:0"/>
			        <feature var="takina:push:encrypt:0"/>
			        <feature var="takina:push:encrypt:aes-128-gcm"/>
			        <feature var="takina:push:filter:ignore-unknown:0"/>
			        <feature var="takina:push:filter:groupchat:0"/>
			        <feature var="takina:push:filter:muted:0"/>
			        <feature var="urn:xmpp:mam:2"/>
			        <feature var="takina:push:priority:0"/>
			        <feature var="takina:push:jingle:0"/>
		...
		val info = takina.getModule(DiscoveryModule).buildInfo(stanza)

		assertTrue("Verification string should be valid!") {
			takina.getModule(EntityCapabilitiesModule).validateVerificationString(info)
		}
	}

	@Test
	fun test_validateVerificationString() {
		val info = takina.getModule(DiscoveryModule).buildInfo(iq {
				type = IQType.Result
	...
	fun testCalculateVerificationString() {
		val module = EntityCapabilitiesModule(takina, DiscoveryModule(takina), StreamFeaturesModule(takina))
