...
actual class TakinaEventBus actual constructor(context: TakinaContext) : AbstractTakinaEventBus(context) {
    ...
    override fun emit(takinaEvent: TakinaEvent) {
        ...
        LogUtils.debug(TAG, "发射事件", eventType.clzName, describeEvent(takinaEvent))
    ...
    private fun describeEvent(event: TakinaEvent): String =
        when (event) {
            is ConnectionConnectedEvent -> "账号=${event.jid}"
            is ConnectionFailedEvent -> "账号=${event.jid} 原因=${event.reason}"
            is AllConnectedEvent -> "成功=${event.connectedCount}/${event.configuredCount}"
            is AllDisconnectedEvent -> "连接数=${event.connectionCount}"
            is StanzaReceivedEvent -> "账号=${event.jid} 类型=${event.stanzaType}"
            is ConnectionClosedEvent -> "账号=${event.jid} 原因=${event.reason}"
            is ConnectionStageChangedEvent -> "账号=${event.jid} 阶段=${event.oldStage} -> ${event.newStage}"
            else -> ""
        }
}
