...
sealed class PubSubItemEvent(
	...
	companion object : EventDefinition<PubSubItemEvent> {
		...
		override val TYPE = "lib.takina.core.xmpp.modules.pubsub.PubSubEventReceivedEvent"
	}
}
...
//	private val log = LoggerFactory.logger("lib.takina.core.xmpp.modules.pubsub.PubSubModule")
	...
	fun publish(
		...
		return context.request.iq(iq).map { resp ->
			val publish = resp.getChildrenNS("pubsub", XMLNS)?.getFirstChild("publish")
				?: throw TakinaException("No publish element")
			val item = publish.getFirstChild("item") ?: throw TakinaException("No item element")
			val j = resp.getFromAttr() ?: throw TakinaException("No sender JID")
			PublishingInfo(
				j,
				publish.attributes["node"] ?: throw TakinaException("No node name"),
				item.attributes["id"] ?: throw TakinaException("No item ID")
			)
		}
	}
