...
fun Element.toBundleOf(jid: BareJID, deviceId: Int): Bundle {
    if (this.name != "bundle" || this.xmlns != OMEMOModule.XMLNS) throw TakinaException("Not a bundle element.")
    val signedPreKeyId = this.getFirstChild("signedPreKeyPublic")?.attributes?.get("signedPreKeyId")
        ?: throw TakinaException("Incomplete bundle: signedPreKeyId not found")
    val signedPreKeyPublic = this.getFirstChild("signedPreKeyPublic")?.value
        ?: throw TakinaException("Incomplete bundle: signedPreKeyPublic not found")
    val signedPreKeySignature = this.getFirstChild("signedPreKeySignature")?.value
        ?: throw TakinaException("Incomplete bundle: signedPreKeySignature not found")
    val identityKey = this.getFirstChild("identityKey")?.value
        ?: throw TakinaException("Incomplete bundle: identityKey not found")

    val prekeys = this.getFirstChild("prekeys")
        ?.getChildren("preKeyPublic")
        ?.map {
            PreKey(
                it.attributes["preKeyId"]?.toInt() ?: throw TakinaException("Invalid preKeyPublic"),
                it.value?.fromBase64() ?: throw TakinaException("Invalid preKeyPublic")
            )
        } ?: emptyList()
    return Bundle(
        jid = jid,
