...
package lib.takina.core.xmpp.modules.chatmarkers
...
import lib.takina.core.xmpp.modules.uniqueId.getOriginID
...
data class ChatMarkerEvent(val jid: JID, val msgId: String, val stanza: Message, val marker: ChatMarkersModule.Marker) :
	...
	companion object : EventDefinition<ChatMarkerEvent> {
		...
		override val TYPE = "lib.takina.core.xmpp.modules.chatmarkers.ChatMarkerEvent"
	}
}
...
class ChatMarkersModule(override val context: Context) : XmppModule, StanzaInterceptor, ChatMarkersModuleConfig {
	...
	override fun beforeSend(element: Element): Element {
		...
		if (isMarkable(to)) element.add(lib.takina.core.xml.element("markable") { xmlns = XMLNS })
 ...
 */
fun MessageNode.markable() = this.element.add(lib.takina.core.xml.element("markable") {
