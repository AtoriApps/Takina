...
import lib.takina.core.requests.RHandler
...
) : TakinaModule, OMEMOModuleConfig {

    /**
     * Module is implementing OMEMO Encryption ([XEP-0384](https://xmpp.org/extensions/attic/xep-0384-0.3.0.html) version 0.3.0).
     */
    companion object : TakinaModuleProvider<OMEMOModule, OMEMOModuleConfig> {
        ...
        internal const val ENCRYPTION_ELEMENT = "takina:omemo"
...
//                throw TakinaException("The `store` parameter is required in OMEMOModule configuration.")
//            }
//            try {
//                module.sessionStore
//            } catch (e: UninitializedPropertyAccessException) {
...
//            moduleManager.registerInterceptors(arrayOf(module))
    ...
    private val log = LoggerFactory.logger("lib.takina.core.xmpp.modules.omemo.OMEMOModule")
...
//            }
    ...
    fun deleteBundle(bundleId: Int): RequestBuilder<Unit, IQ> = pubsubModule.deleteNode(
        jid = context.boundJID?.bareJID ?: throw TakinaException("Entity not bound"),
    ...
    fun deleteDeviceList(): RequestBuilder<Unit, IQ> = pubsubModule.deleteNode(
        jid = context.boundJID?.bareJID ?: throw TakinaException("Entity not bound"), node = DEVICE_LIST_NODE
    )
    ...
    fun publishDeviceList(deviceIds: List<Int>): PublishRequestBuilder<PubSubModule.PublishingInfo> {
        ...
        return publish(
            jid = context.boundJID?.bareJID ?: throw TakinaException("JID not bound."),
...
//            else handler.invoke(Result.failure(TakinaException("Cannot create OMEMO session")))
//        }
//    }
//
//    /**
...
//                handler(Result.success(s))
//            }
//            it.onFailure { handler(Result.failure(TakinaException("Cannot create session"))) }
//        }
//    }
//
...
fun OMEMOModule.addDevice(deviceId: Int) {
    val jid = context.boundJID?.bareJID ?: throw TakinaException("JID not bound.")
    retrieveDevicesIds(jid).response {
...
private fun Element.encryptionOrder(): EncryptMessage {
    ...
    return when (e.attributes["encryption"]) {
        ...
        else -> throw TakinaException("Invalid OMEMO control element.")
    }
}
