...
import org.atoriapps.takina.core.IContext
...
class 恩情Component : TakinaComponent {
    companion object : TakinaComponentProvider<恩情Component> {
        ...
        override fun getInstance(context: IContext): 恩情Component {
            LogUtils.debug("恩情Component", "将军从丹东来，换我一城雪白，想吃广东菜，获取实例")
            return 恩情Component()
        }

        override fun configure(context: IContext, component: 恩情Component) {
...
interface TakinaComponentProvider<out C : TakinaComponent> {
    fun getInstance(context: IContext): C

    // 默认空体实现
    fun configure(context: IContext, component: @UnsafeVariance C) {}
}
