package org.atoriapps.takina.core.api
...
import org.atoriapps.takina.core.connection.ConnectionConfig
...
import org.atoriapps.takina.core.connection.XmppTransport
import org.atoriapps.takina.core.connection.XmppTransportCallbacks
import org.atoriapps.takina.core.connection.XmppTransportFactoryRegistry
...
import org.atoriapps.takina.core.feature.FeatureContext
...
interface Takina {
    ...
    fun forAccount(jid: BareJid): AccountHandle
...
class AccountHandle internal constructor(
    ...
    fun forPeer(peer: BareJid): DirectChatHandle = DirectChatHandle(this, peer)
    fun forRoom(room: BareJid): MucHandle = MucHandle(this, room)
}
...
class DirectChatHandle internal constructor(
    ...
    fun message(init: org.atoriapps.takina.core.request.MessageRequestDsl.() -> Unit) = account.request.message {
        to = peer
        init()
    }
}
...
internal class CoreTakina(
...
) : Takina, RequestExecutor, FeatureContext {
    ...
    init {
        ...
        installedFeatures.forEach { feature ->
            ...
            if (feature is ApiProvidingFeature<*>) {
                featureApisByKey[feature.apiKey.featureKey] = feature.api()
            }
        }
    ...
    override fun removeAccount(jid: BareJid) {
        ...
        require(connection == ConnectionState.IDLE || connection == ConnectionState.CLOSED) {
            "Account must be detached from active connection before removal: $jid"
        }
    ...
    override fun forAccount(jid: BareJid): AccountHandle {
        ensureStarted()
        require(accounts.containsKey(jid)) { "Account not found: $jid" }
        return AccountHandle(this, jid)
    }

    override fun <API : FeatureApi> api(key: FeatureApiKey<API>): API =
        requireNotNull(apiOrNull(key)) { "Feature API not available: ${key.featureKey}" }
    ...
    override suspend fun connect(jid: BareJid) {
        ...
        val transport = XmppTransportFactoryRegistry.factory(
            account.toConnectionConfig(accountScope),
    ...
    override suspend fun emitFeatureEvent(eventName: String, payload: Map<String, String>) {
        events.emit(FeatureStateChangedEvent(eventName, payload["enabled"]?.toBooleanStrictOrNull() ?: true))
    }
    ...
    suspend fun onUnexpectedDisconnect(owner: BareJid, reason: String?, authHardFailure: Boolean) {
        ...
        val reconnect = ReconnectOrchestrator(
            smRecoveryCoordinator = object : SmRecoveryCoordinator {
                override suspend fun tryResume(owner: BareJid): SmResumeResult = SmResumeResult.NOT_ENABLED
            },
        ...
        val outcome = reconnect.recoverAfterUnexpectedDisconnect(owner, policy, authHardFailure = authHardFailure)
    ...
    private suspend fun handleInboundFrame(owner: BareJid, frame: String) {
        ...
        when (classification) {
            org.atoriapps.takina.core.pipeline.InboundClassification.STANZA_MESSAGE -> {
                if (parsed == null) {
            ...
            org.atoriapps.takina.core.pipeline.InboundClassification.STANZA_PRESENCE -> {
                if (parsed == null) {
            ...
            org.atoriapps.takina.core.pipeline.InboundClassification.STANZA_IQ -> {
                if (parsed == null) {
            ...
            org.atoriapps.takina.core.pipeline.InboundClassification.UNKNOWN -> {
    ...
    private fun AccountDefinition.toConnectionConfig(scope: Scope): ConnectionConfig {
